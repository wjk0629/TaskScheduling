function [output_origin,output_set,robotTaskSeq_set] = geneticInitial(taskSet,robotTaskSeq,output_origin,pop_size,gridMap,taskXY,stationXY,palletXY)
load('mapGeneration');
output_set = [];
...
for population = 1:pop_size
    ...
    if (mod(population,3) + 1) == 3
        changeTaskIdx = 1;
    else
        changeTaskIdx = mod(population,3)+2;
    end
    ...
    if population ~= 1
        ...
        %     for j = 1:individual
        %         if taskSet(find(robotTaskSeq{(mod(population,3)+1),1}(j)==taskSet(:,2)),1) == individualTaskCount(mod(population,3)+1)*alpha
        %             robotTask_temp1= robotTaskSeq{(mod(population,3)+1),1}(j);
        %             robotTaskSeq{(mod(population,3)+1),1}(j) = robotTaskSeq{changeTaskIdx,1}(j);
        %             robotTaskSeq{changeTaskIdx,1}(j) = robotTask_temp1;
        
        %             robotTask_temp2= robotTaskSeq{(mod(population,3)+1),2}(j);
        %             robotTaskSeq{(mod(population,3)+1),2}(j) = robotTaskSeq{changeTaskIdx,2}(j);
        %             robotTaskSeq{changeTaskIdx,2}(j) = robotTask_temp2;
        %             individualTaskCount(mod(population,3)+1) = individualTaskCount(mod(population,3)+1) + 1;
        %             fprintf('%d robot의 %d번째 작업과 %d robot의 %d번째 작업이 바뀌었습니다.\n', mod(population,3)+1, j, changeTaskIdx, j)
        ...
        %         end
        %     end
        randomTaskSet = randperm(length(taskSet(:,2)));
        randomTaskSet = reshape(randomTaskSet,nRobot,length(randomTaskSet)/3);
        for j = 1:nRobot
            robotTaskSeq{j,1} = randomTaskSet(j,:)';
            robotTaskSeq{j,2} = taskSet(robotTaskSeq{j,1},3);
        end
        robotTaskSeq_set = [robotTaskSeq_set; robotTaskSeq];
    end
    ...
    fprintf('** %d pop_size 중 %d 번 실행했습니다. **\n', pop_size-1, population)
    tStep = 0;
...
fprintf(' Random initial generation complete , best -> time : %f \n',min(output_set(4,:)))
end
