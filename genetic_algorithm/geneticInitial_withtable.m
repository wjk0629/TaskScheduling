function [output_origin,output_set,robotTaskSeq_set] = geneticInitial_withtable(taskSet,robotTaskSeq,output_origin,pop_size,gridMap,taskXY,stationXY,palletXY)
load('mapGeneration');
output_set = [];
...
for population = 1:pop_size
    ...
    if population ~= 1
        randomTaskSet_temp = randperm(length(taskSet(:,2)));
        random_mod = mod(length(randomTaskSet_temp),nRobot);
        randomTaskSet = reshape(randomTaskSet_temp(1:end-random_mod),nRobot,(length(randomTaskSet_temp)-random_mod)/nRobot);
        randomTaskSet_mod = randomTaskSet_temp(end-random_mod+1:end);
        for j = 1:nRobot
            robotTaskSeq{j,1} = randomTaskSet(j,:)';
            robotTaskSeq{j,2} = taskSet(robotTaskSeq{j,1},3);
        end
        if ~isempty(randomTaskSet_mod)
            random_mod2 = randperm(nRobot,1);
            for k = 1:length(randomTaskSet_mod)
                robotTaskSeq{random_mod2,1} = [robotTaskSeq{random_mod2,1}; randomTaskSet_mod(k)];
                robotTaskSeq{random_mod2,2} = [robotTaskSeq{random_mod2,2}; taskSet(robotTaskSeq{random_mod2,1}(end),3)];
            end
        end
...
fprintf(' Random initial generation complete , best -> bundle : %f \n',max(output_set(1,:)))
