...
gridMap = ones(xSize,ySize);
for x = 1:xSize
    for y = 1:ySize
        if x >= 1 && x < 21
            if y>=1&&y<3 || y==5 || y==7 || y==9 || y==15 || y== 19 
                gridMap(x,y) = 0;
            end
        end
        if x >= 2 && x < 4
...
nStation = 3;
stationXY = [20, 5; 20, 7; 20, 9]; % map의 오른쪽 끝
for i = 1:length(stationXY(:,1))
    gridMap(stationXY(i,1),stationXY(i,2)) = 0;     
end

palletXY = [1,5; 1,7];
for i = 1:length(palletXY(:,1))
    gridMap(palletXY(i,1),palletXY(i,2)) = 0;     
end

nTask = 6;
% taskXY = [];
% for i=1:nTask
%     taskXY = [taskXY; i (ceil(xSize/nTask)*i-1)  ySize-1 0];
% end
taskXY = [1,3,16,0; 2,3,18,0; 3,15,16,0; 4,15,18,0; 5,18,16,0; 6,18,18,0];
for i = 1:length(taskXY(:,1))
    gridMap(taskXY(i,2),taskXY(i,3)) = 0;     
end
taskSetTime = [20 35 50 20 35 50];
...
while tStep < tMax
    ...
    for i = 1:length(workingRobotIdx)
        ...
        if (~conflict) || (conflict && ~makeDetour)
            ...
            robotSet(thisRobotIdx,6) = thisRobotAngle;
            thisRobotWaitingCell = thisRobotPath(1,:);
            thisRobotPath(1,:) = [];
        ...
        % 속도,각도 추가한 부분
        thisVeloInfo = [thisRobotPath(1,1)-thisRobotXY(1) thisRobotPath(1,2)-thisRobotXY(2)];
        thisRobotAngle = atan2(thisVeloInfo(2),thisVeloInfo(1));
        
        % Check angle change
        if (thisRobotAngle - robotSet(thisRobotIdx,6)) == pi/2
            thisRobotPath = [thisRobotWaitingCell; thisRobotWaitingCell; thisRobotPath];
            turnIdx = 1;
        elseif (thisRobotAngle - robotSet(thisRobotIdx,6)) == pi/4
            thisRobotPath = [thisRobotWaitingCell; thisRobotWaitingCell; thisRobotWaitingCell; thisRobotWaitingCell; thisRobotPath];
            turnIdx = 2;
        end
