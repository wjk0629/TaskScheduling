...
iteration = 1;
output_set = [];
showFig = 2; % 0으로 하면 test만 2로 하면 plot까지 한다. 
nRobot = 3;
numTaskPerStep = 4;
...
taskXY = readmatrix('Map_2.xlsx','Sheet',station_sheet,'Range','A3:D8');
...
[taskSet,robotTaskSeq,output_origin] = tasksetting2(nRobot,numTaskPerStep,alpha,gridMap,tMax,bSize);

robotTaskSeq_origin = robotTaskSeq;
individualTaskCount = [0 0 0]; % 작업 순서를 바꿔주기 위하여 만든 인덱스

fprintf('*********************   test   start   *********************\n\n');
fprintf('greedy algorithm으로 생긴 task_origin 순서는 다음과 같습니다.\n');
fprintf('1번 로봇 : ');disp([robotTaskSeq_origin{1,1}]');%fprintf('\n');
fprintf('2번 로봇 : ');disp([robotTaskSeq_origin{2,1}]');%fprintf('\n');
fprintf('3번 로봇 : ');disp([robotTaskSeq_origin{3,1}]');%fprintf('\n');
fprintf('************************************************************\n\n');
...
if showFig > 0
    ...
    plot(palletXY(:,1), palletXY(:,2), 'gs', 'linewidth', 1, 'markersize',20, 'MarkerEdgeColor','g', 'MarkerFaceColor','g')
    plot(warehouseXY(:,1), warehouseXY(:,2), 'ks', 'linewidth', 1, 'markersize',20, 'MarkerEdgeColor','k', 'MarkerFaceColor','k')
end
...
thisRobotTaskSeq2Idx = [1 1 1]; % 정해진 작업보다 더 많은 작업을 하기위해 추가한 인덱스


if (mod(iter,3) + 1) == 3
    changeTaskIdx = 1;
else
    changeTaskIdx = mod(iter,3)+2;
end
...
if iter ~= 1
    for j = 1:individual
        if taskSet(find(robotTaskSeq{(mod(iter,3)+1),1}(j)==taskSet(:,2)),1) == individualTaskCount(mod(iter,3)+1)*alpha
            robotTask_temp1= robotTaskSeq{(mod(iter,3)+1),1}(j);
            robotTaskSeq{(mod(iter,3)+1),1}(j) = robotTaskSeq{changeTaskIdx,1}(j);
            robotTaskSeq{changeTaskIdx,1}(j) = robotTask_temp1;
            robotTask_temp2= robotTaskSeq{(mod(iter,3)+1),2}(j);
            robotTaskSeq{(mod(iter,3)+1),2}(j) = robotTaskSeq{changeTaskIdx,2}(j);
            robotTaskSeq{changeTaskIdx,2}(j) = robotTask_temp2;
            individualTaskCount(mod(iter,3)+1) = individualTaskCount(mod(iter,3)+1) + 1;
            fprintf('%d robot의 %d번째 작업과 %d robot의 %d번째 작업이 바뀌었습니다.\n', mod(iter,3)+1, j, changeTaskIdx, j)
            fprintf('현재 작업 순서는 다음과 같습니다.\n');
            fprintf('1번 로봇 : ');disp([robotTaskSeq{1,1}]');%fprintf('\n');
            fprintf('2번 로봇 : ');disp([robotTaskSeq{2,1}]');%fprintf('\n');
            fprintf('3번 로봇 : ');disp([robotTaskSeq{3,1}]');%fprintf('\n');
            break;
        else
            %individualTaskCount(mod(iter,3)+1) = individualTaskCount(mod(iter,3)+1) + 1;
        end
...
while tStep < tMax
    ...
    for i = 1:length(idleRobotIdx)
        ...
        thisRobotwarehouseXYIdx = 0;
        if thisRobotwarehouseXYIdx == 0
            thisRobotwarehouseXY = warehouseXY(1,:);
            thisRobotwarehouseXYIdx = 1;
        else
            thisRobotwarehouseXY = warehouseXY(2,:);
            thisRobotwarehouseXYIdx = 0;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %일단 사전 작업이 필요하다
...
%             end
        ...
        if ~isempty(taskQueue) && size(thisRobotBundle,1) < bSize
            ...
            if size(taskQueue,1) >= bFill
                ...
                %                     thisRobotPredictedTime(j) = thisRobotPredictedTime(j) + length(flipud(astar(taskQueue(j,4:5),thisRobotwarehouseXY,~gridMap))); % warehouse까지의 거리(=시간)을 더해준 값
                %                 end
                ...
                %                 thisRobotPredictedTime = thisRobotPredictedTime + length(flipud(astar(taskQueue(j,4:5),thisRobotwarehouseXY,~gridMap))); % warehouse까지의 거리(=시간)을 더해준 값
    ...
    % Once all items are loaded, the robot goes to the packing warehouse and stays one step for unloading.
    ...
    for i = 1:length(workingRobotIdx)
        ...
        if conflict && makeDetour
            ...
            startXY = thisRobotXY;
            thisRobotwarehouseXY = warehouseXY(thisRobotIdx,:);
        ...
        if isempty(thisRobotPath)%if the current task is done, now need to see the bundle for the rest of tasks in the bundle
            ...
            if isempty(thisRobotBundle)%if bundle is empty
                if robotSet(thisRobotIdx,4) == 2
                    robotSet(thisRobotIdx,4) = 1;%mark this robot going to packing warehouse
                    taskXY(robotSet(thisRobotIdx,5),4) = 0;
                    taskSetTime(robotSet(thisRobotIdx,5)) = 0;
                    robotSet(thisRobotIdx,5) = 0;
                    %plan path to the warehouse
                    startXY = thisRobotXY;
                    % 일단 warehouse 두개 번갈아 설정하도록 해놓음 나중에 바꿔도됨
                    if mod(tStep,2) == 0
                        thisRobotwarehouseXY = warehouseXY(1,:);
                    else
                        thisRobotwarehouseXY = warehouseXY(2,:);
...
tStep = 0; 
output = [numCompletedTasks; timeRobotMove/(numCompletedTasks); timeInBundle/numBundledTasks; completionTimeAllTasks/numAllTasks; completionTimeDoneTasks/numCompletedTasks; timeInQueueTasks/numAllTasks; (timeInQueueTasks + timeInBundle + timeInExecution)/numAllTasks;];
if iter == 1
