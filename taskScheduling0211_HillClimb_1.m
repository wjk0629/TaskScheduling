...
while tStep < tMax
    ...
    for i = 1:length(idleRobotIdx)
        ...
        if thisRobotTaskSeq2Lth >= thisRobotTaskSeq2Idx(thisRobotIdx)
            thisRobotTaskTemp1 = find(robotTaskSeq{thisRobotIdx,2}(thisRobotTaskSeq2Idx(thisRobotIdx))==taskSet(:,3));
            thisRobotTaskTemp2 = taskSet(thisRobotTaskTemp1,1);
            ...
            for j = 1:length(thisRobotTaskTemp2)
                if thisRobotTaskTemp2(j) + thisRobotPredictedTime >= tStep - alpha && thisRobotTaskTemp2(j) + thisRobotPredictedTime < tStep + alpha
                    thisRobotTask = taskSet(thisRobotTaskTemp1(j),:);
