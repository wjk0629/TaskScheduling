...
pop_size = 11; % 원하는 수 + 1해야됨 나중에 수정할 것
nRobot = 3;
numTaskPerStep = 2;
alpha = 10; % task가 오는 텀
tMax = 300;
...
while iteration_count < iteration
    ...
    for i = 1:length(output_set(1,:)) %% 복제 수량의 확률
        output_proba_bTemp(1,i) = output_set(1,i)/fitness_bundle;
        output_proba_tTemp(1,i) = output_set(7,i)/fitness_time;
        output_proba_ttTemp(1,i) = fitness_mytest(i)/fitness_test;
    end
    %% Remainder stochastic sampling with replacement
%     output_proba = output_proba_ttTemp.*10; % 복제 수량의 기대치
%     output_proba2 = floor(output_proba); % 정수부 만큼 내림
%     for i = 1:length(output_proba)
%         if output_proba2(i) >= 1
%            output_proba2(i) 
%             
%             
%         end
...
%     end
    ...
    if isempty(chromosome_index)
        ...
        for i = 1:length(chromosome_index)/2
            ...
            if  isequal(robotTaskSeq_chro_selected1{chromosome_pair_index2},robotTaskSeq_chro_selected2{chromosome_pair_index2}) == 1
                disp("warning! there is no crossover (pair is same)");
    ...
    Forplotdata_y = [Forplotdata_y, min(output_set(4,:))];
